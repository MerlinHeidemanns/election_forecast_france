...
sim_fundamentals_ordered <- function(NElections, NBlocs){
  ...
  prob_mu[1, ] <- exp_softmax(
    logodds_mu[1, ] +
    c(x_fundamentals[1,] %*% beta_fundamentals)[incumbency_distance[incumbency[1, ],]])
  ...
  for (tt in 2:NElections){
    ...
    logodds_mu[tt, ] <- logodds_alpha[tt, ] +
      ...
      c(x_fundamentals[tt,] %*% beta_fundamentals)[incumbency_distance[incumbency[1, ],]]
  ...
  out_list <- list(
    ...
    beta_fundamentals = data.frame(name = c(rep("popularity", 6), rep("unemployment", 6)),
                                  value = c(beta_fundamentals[1,], beta_fundamentals[2,]),
                                  bloc_id = rep(seq(1, 6), 2)),
    incumbency_distance = incumbency_distance
  )
}
