...
m2_miss <- rbind(m2_miss, c(1, 2, 3, 5, 6, 7, 0))
...
data_list <- list(
  ...
  miss_results = miss_results,
  abstention_share_results = result_abstention,
  P = df %>% distinct(election_id, pollster_id) %>%
...
################################################################################
## Model
mod <- cmdstan_model("src/stan/testing/gp/gp_multi_election_season_hist_prior_v3.stan")
################################################################################
## Run
fit <- mod$sample(
  data = data_list,
