...
library(tidyverse)
## Load data
df <- read_csv("dta/polls_dta/2012_polls_compiled.csv")
...
candidates_crosswalk <- data.frame(short_name = candidates,
           long_name = NA)
election_results_2012 <- read_csv("dta/polls_dta/election_results_clean.csv") %>%
  filter(year == 2012)
for (j in 1:nrow(candidates_crosswalk)){
  if (any(grepl(candidates_crosswalk[j, 1],
                election_results_2012$candidate))){
    candidates_crosswalk[j, 2] = election_results_2012$candidate[grepl(candidates_crosswalk[j, 1],
                                                                       election_results_2012$candidate)]
  }
}
...
er_candidate_count <- nrow(election_results_2012)
...
write.csv(candidate_short_id_df, "dta/polls_dta/candidate_identifiers_2012_short.csv")
candidate_long_id_df <-df %>%
  distinct(long_name) %>%
  arrange(long_name) %>%
  filter(!is.na(long_name)) %>%
  mutate(candidate_long_id = 1:n())
write.csv(candidate_long_id_df, "dta/polls_dta/candidate_identifiers_2012_long.csv")
df <- df %>%
...
write.csv(pollster_id_df, "dta/polls_dta/pollster_identifiers_2012.csv")
df <- df %>%
...
write.csv(df, file = "dta/polls_dta/2012_polls_clean.csv")
