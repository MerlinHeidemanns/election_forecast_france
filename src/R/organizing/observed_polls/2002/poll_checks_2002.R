...
library(tidyverse)
## Load data
df <- read_csv("dta/polls_dta/2002_polls_compiled.csv")
...
candidates_crosswalk <- data.frame(short_name = candidates,
           long_name = NA)
election_results_2002 <- read_csv("dta/polls_dta/election_results_clean.csv") %>%
  filter(year == 2002)
for (j in 1:nrow(candidates_crosswalk)){
  if (any(grepl(candidates_crosswalk[j, 1],
                election_results_2002$candidate))){
    candidates_crosswalk[j, 2] = election_results_2002$candidate[grepl(candidates_crosswalk[j, 1],
                                                                       election_results_2002$candidate)]
  }
}
if (all(!grepl("Abstention", candidates_crosswalk$long_name))){
  candidates_crosswalk <- rbind(
    candidates_crosswalk,
    data.frame(short_name = "_Abstention",
               long_name = "_Abstention")
  )
}


candidate_count <- candidates_crosswalk %>%
  pull(long_name) %>%
  is.na()
cw_candidate_count <- sum(candidate_count == FALSE)
er_candidate_count <- nrow(election_results_2002)
if (cw_candidate_count != er_candidate_count){
  stop("Problem with name matching for candidates.")
}
...
candidate_short_id_df <- df %>%
  distinct(candidate) %>%
  add_row(candidate = "_Abstention") %>%
  arrange(candidate) %>%
  mutate(candidate_short_id = 1:n())
write.csv(candidate_short_id_df, "dta/polls_dta/candidate_identifiers_2002_short.csv")
candidate_long_id_df <- df %>%
  add_row(long_name = "_Abstention") %>%
  distinct(long_name) %>%
  arrange(long_name) %>%
  filter(!is.na(long_name)) %>%
  mutate(candidate_long_id = 1:n())
write.csv(candidate_long_id_df, "dta/polls_dta/candidate_identifiers_2002_long.csv")
df <- df %>%
...
write.csv(pollster_id_df, "dta/polls_dta/pollster_identifiers_2002.csv")
df <- df %>%
...
write.csv(df, file = "dta/polls_dta/2002_polls_clean.csv")
