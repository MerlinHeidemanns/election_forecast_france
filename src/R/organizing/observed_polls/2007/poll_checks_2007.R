...
library(tidyverse)
## Load data
df <- read_csv("dta/polls_dta/2007_polls_compiled.csv")
...
candidates_crosswalk <- data.frame(short_name = candidates,
           long_name = NA)
election_results_2007 <- read_csv("dta/polls_dta/election_results_clean.csv") %>%
  filter(year == 2007)
for (j in 1:nrow(candidates_crosswalk)){
  if (any(grepl(candidates_crosswalk[j, 1],
                election_results_2007$candidate))){
    candidates_crosswalk[j, 2] = election_results_2007$candidate[grepl(candidates_crosswalk[j, 1],
                                                                       election_results_2007$candidate)]
  }
}
...
er_candidate_count <- nrow(election_results_2007)
...
write.csv(candidate_short_id_df, "dta/polls_dta/candidate_identifiers_2007_short.csv")
...
write.csv(candidate_long_id_df, "dta/polls_dta/candidate_identifiers_2007_long.csv")
df <- df %>%
...
write.csv(pollster_id_df, "dta/polls_dta/pollster_identifiers_2007.csv")
df <- df %>%
...
## Add election year
df <- df %>%
  mutate(election_year = 2007)

write.csv(df, file = "dta/polls_dta/2007_polls_clean.csv")
