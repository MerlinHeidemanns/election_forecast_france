...
return_overview_sites <- function(){
  ...
  links <- sapply(years, function(x){
    return(paste("http://www.politiquemania.com/regionales-",
...
site <- return_sites(return_overview_sites()$links[1])[1]
return_results <- function(site){
  ...
  ## Departement
  departement <- read_html(site) %>%
    html_node("body") %>%
    xml2::xml_find_all("//h1[contains(@class, 'title')]") %>%
    html_text() %>%
    str_match(., "\\:\\s(.+)")
  ...
  labels <- read_html(site) %>%
    ...
    xml2::xml_find_all("//table[contains(@class, 'resultats')]")%>%
    `[[`(1) %>%
    xml2::xml_find_all("//tr[contains(@colspan, '4'")
...
candidate_tables_df <- candidate_tables_df %>%
  ...
  mutate(insee_code = as.character(insee_code))
## Save
write_csv(total_tables_df, "dta/election_results/regional_department_total_votes.csv")
write_csv(candidate_tables_df, "dta/election_results/regional_department_candidate_votes.csv")
###############################################################################