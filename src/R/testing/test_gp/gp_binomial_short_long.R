...
mod <- cmdstan_model("src/stan/testing/gp/gp_binomial_short_long_sim.stan")
data_list <- list(N = 1000,
                  T = 500,
                  alpha_short = 0.5,
                  alpha_long = 0.1,
                  length_scale_short = 10,
                  length_scale_long = 100
)
set <- sample(1:data_list$N,size = 40, replace = F)
fit <- mod$sample(
  data = data_list,
  chains = 6,
  iter_sampling = 1,
...
ggplot(data = plt_df[set,], aes(x=x, y=y/n)) +
  ...
  ylab('y')
## Load model
mod_gp <- cmdstan_model("src/stan/testing/gp/gp_binomial_short_long.stan")
## Datalist
