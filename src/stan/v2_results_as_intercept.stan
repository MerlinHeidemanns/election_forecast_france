  ...
  profile ("sum to zero constraints past"){
    ...
    for (jj in 1:(NElections - 1)){
      tau_past[, (1 + supvec_NPolls_past[jj]):(NPolls_past[jj] + supvec_NPolls_past[jj])] =
        raw_tau_past[, (1 + supvec_NPolls_past[jj]):(NPolls_past[jj] + supvec_NPolls_past[jj])] * sigma_tau;
      for (ii in 2:NBlocs){
...
model {
  ...
  // // Past elections
  // // * Scale parameters
  to_vector(raw_alpha_past) ~ normal(0, 1);
  to_vector(raw_xi_past) ~ normal(0, 1);
  to_vector(raw_tau_past) ~ normal(0, 1);
