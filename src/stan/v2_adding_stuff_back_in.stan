...
transformed parameters {
  ...
  profile("sum to zero current") {
    ...
    alpha[1] = rep_vector(0.0, NPollsters)';
    for (ii in 2:NCandidates) alpha[ii, 1] = -sum(alpha[ii, 2:NPollsters]);
    for (ii in 1:NPollsters) alpha[1 + abstention_omitted_pollster[ii], ii]  = -sum(alpha[2 + abstention_omitted_pollster[ii]:NCandidates, ii]);
    // Nonsampling error
    tau = raw_tau * sigma_tau;
    tau[1] = rep_vector(0.0, NSurveys)';
    for (jj in 1:NPollsters){
      // tau[, (1 + supvec_NSurveys_Pollsters[jj]):(NSurveys_Pollster[jj] + supvec_NSurveys_Pollsters[jj])] =
      //   raw_tau[, (1 + supvec_NSurveys_Pollsters[jj]):(NSurveys_Pollster[jj] + supvec_NSurveys_Pollsters[jj])] * sigma_tau;
      ...
      for (ii in (1 + supvec_NSurveys_Pollsters[jj]):(NSurveys_Pollster[jj] + supvec_NSurveys_Pollsters[jj])){
        tau_past[1 + abstention_omitted[ii], ii] =
          -sum(tau_past[2 + abstention_omitted[ii]:NBlocs, ii]);
      }
    }
  }
  ...
  profile ("sum to zero constraints past"){
    ...
    alpha_past[1] = rep_vector(0.0, sum(NPollsters_past))';
    for (jj in 1:(NElections - 1)){
      ...
      for (ii in (1 + supvec_NPollsters_past[jj]):(NPollsters_past[jj] + supvec_NPollsters_past[jj])){
        alpha_past[1 + abstention_omitted_pollster_past[ii], ii] = -sum(alpha_past[2 + abstention_omitted_pollster_past[ii]:NBlocs, ii]);
      }
    }
    ...
    //   tau_past[, (1 + supvec_NPolls_Pollster_past[jj]):(NPolls_Pollster_past[jj] + supvec_NPolls_Pollster_past[jj])] =
    //     raw_tau_past[, (1 + supvec_NPolls_Pollster_past[jj]):(NPolls_Pollster_past[jj] + supvec_NPolls_Pollster_past[jj])] * sigma_tau;
    //   for (ii in 2:NBlocs){
   ...
   tau_past[1] = rep_vector(0.0, sum(NPolls_Pollster_past))';
   for (jj in 1:sum(NPollsters_past)){
      for (ii in 2:NBlocs){
        tau_past[ii, 1 + supvec_NPolls_Pollster_past[jj]] =
          - sum(tau_past[ii, (2 + supvec_NPolls_Pollster_past[jj]):(NPolls_Pollster_past[jj] + supvec_NPolls_Pollster_past[jj])]);
      ...
      for (ii in (1 + supvec_NPolls_Pollster_past[jj]):(NPolls_Pollster_past[jj] + supvec_NPolls_Pollster_past[jj])){
        tau_past[1 + abstention_omitted_past[ii], ii] =
          -sum(tau_past[2 + abstention_omitted_past[ii]:NBlocs, ii]);
      }
    }
...
model {
  ...
  // // Past elections
  //sigma_cov_blocs ~ normal(0, prior_sigma_cov);
  //chol_corr_theta_blocs ~ lkj_corr_cholesky(10.0);
    ...
    for (ii in 1:NPolls){
      {
        ...
        prob_theta_subset = prob_theta_complete;
