...
parameters {
  ...
  real<lower = lsigma> sigma_tau_past;
  real<lower = lsigma> sigma_alpha_past;
  real<lower = lsigma> sigma_xi_past;
...
transformed parameters {
  ...
  theta_candidates_prior = prior_theta_candidates;
...
model {
  ...
  sigma_xi_past ~ normal(0, prior_sigma_xi);
  sigma_alpha_past ~ normal(0, prior_sigma_alpha);
  sigma_tau_past ~ normal(0, prior_sigma_tau);
  ...
  // // Past elections
  // // * Scale parameters
  to_vector(raw_alpha_past) ~ normal(0, sigma_alpha_past);
  to_vector(raw_xi_past) ~ normal(0, sigma_xi_past);
  to_vector(raw_tau_past) ~ normal(0, sigma_tau_past);
    ...
    for (ii in 1:NElections){
      {
        ...
        theta_election ~ dirichlet(to_vector(elections_results[,ii]));
      }
    }
  }
}
