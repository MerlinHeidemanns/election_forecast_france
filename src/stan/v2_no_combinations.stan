  ...
  profile ("sum to zero constraints past"){
    for (jj in 1:(NElections - 1)){
      alpha_past[, (1 + supvec_NPollsters_past[jj]):(NPollsters_past[jj] + supvec_NPollsters_past[jj])] =
        raw_alpha_past[, (1 + supvec_NPollsters_past[jj]):(NPollsters_past[jj] + supvec_NPollsters_past[jj])] * sigma_alpha;
      for (ii in 2:NBlocs){
    ...
    for (ii in 1:NPolls){
      {
        ...
        //y[(1 + abstention_omitted[ii]):NCandidatesC_ii, ii] ~ binomial(1000,prob_theta_complete[(1 + abstention_omitted[ii]):NCandidatesC_ii]/sum(prob_theta_complete[(1 + abstention_omitted[ii]):NCandidatesC_ii]));
      }
    }
  }
    ...
    for (ii in 1:NElections){
      {
        ...
        logit(to_vector(elections_results[,ii])/sum(to_vector(elections_results[,ii]))) ~ normal(logit(theta_election), 0.01);
      }
    }
  }
}
