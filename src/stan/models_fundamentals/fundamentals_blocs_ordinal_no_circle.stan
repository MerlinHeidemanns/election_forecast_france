...
parameters {
  ordered[NBlocs - 1] thresholds[2, NElections];
  vector<lower = 0, upper = 1>[NDepartments] alpha;
  ...
  for (j in 1:NObs){
    {
      ...
      theta[j, included_blocs[id_Obs_elections[j]][1:NBlocs_Elections[id_Obs_elections[j]]]] =
        alpha[id_Obs_departments[j]] * compute_theta(mu[j], thresholds[1, id_Obs_elections[j], 1:NBlocs_Elections[id_Obs_elections[j]] - 1])' +
        ((1 - alpha[id_Obs_departments[j]]) * compute_theta(mu[j], thresholds[2, id_Obs_elections[j], 1:NBlocs_Elections[id_Obs_elections[j]] - 1])');
    }
  }
}
model {
  ...
  for (j in 1:NElections){
    for (r in 1:2){
      thresholds[r, j] ~ student_t(3, 0, 3);
    }
  }
