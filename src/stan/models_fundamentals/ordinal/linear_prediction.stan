...
parameters {
  ...
  positive_ordered[NBlocs - 1] intercept;
  // Department level predictors
  vector[NMiss_X] XMiss;
  // National coefficients
  vector[NElections] delta;
  vector[K + 1] beta;
  // Error
  matrix[NObs, NBlocs - 1] raw_epsilon;
  vector<lower = 0>[NBlocs - 1] sigma_epsilon;
  // National predictors
...
transformed parameters {
  ...
  y_star = XNation_[id_Obs_elections] * beta + delta[id_Obs_elections];
  epsilon = diag_post_multiply(raw_epsilon, sigma_epsilon);
}
...
model {
  ...
  alpha ~ std_normal();
  beta ~ normal(0, 1);
  to_vector(raw_epsilon) ~ std_normal();
  sigma_epsilon ~ normal(0, 0.5);
  intercept ~ normal(0, 1);
  delta ~ normal(0, 1);
  for (j in 1:NObs){
    {
      vector[NMiss[j]] theta = softmax(append_row(0.0, y_star[j] +
                                                     epsilon[j, miss[j, 1:NMiss[j] - 1]]'));
