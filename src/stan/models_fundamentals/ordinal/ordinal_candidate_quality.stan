...
parameters {
  ...
  // Candidate quality
  matrix[NElections, NBlocs - 1] raw_delta;
  vector<lower = 0>[NBlocs - 1] sigma_delta;
  ...
  vector[NDepartments] raw_disc_departments;
  vector[NElections] raw_disc_elections;
  real<lower = 0> sigma_disc_departments;
  real<lower = 0> sigma_disc_elections;
}
transformed parameters {
  ...
  matrix[NElections, NBlocs - 1] delta = diag_post_multiply(raw_delta, sigma_delta);
  vector[NObs] disc = exp(raw_disc_elections[id_Obs_elections] * sigma_disc_elections); //exp(raw_disc_departments[id_Obs_departments] *sigma_disc_departments) +
  // Department level predictors
  ...
  y_star = alpha[id_Obs_departments] + XDepartment_miss * gamma + psi[id_Obs_elections] + zeta * (1.0 * to_vector(id_Obs_elections));
  for (j in 1:NObs){
    {
      theta[j,
...
model {
  ...
  sigma_disc_departments ~ normal(0, 1);
  sigma_disc_elections ~ normal(0, 1);
  raw_disc_departments ~ normal(0, 1);
  raw_disc_elections ~ normal(0, 1);
...
generated quantities {
  ...
  y_ppc = theta;
  {
    matrix[NElections, NBlocs - 1] rep_delta;
    for (j in 1:NElections){
      rep_delta[j] = to_row_vector(normal_rng(rep_vector(0.0, NBlocs - 1), sigma_delta));
    }
    for (j in 1:NObs){
      y_pred[j,
            included_blocs[id_Obs_elections[j]][1:NBlocs_Elections[id_Obs_elections[j]]]] =
