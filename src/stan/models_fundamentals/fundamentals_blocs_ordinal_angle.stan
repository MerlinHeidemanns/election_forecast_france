functions {
  ...
  vector compute_theta_circle(real mu, vector thres){
    real adjustment = Phi(pi()) - Phi(-pi());
    int len_theta = size(thres) + 1;
    vector[len_theta] out_theta;
    out_theta[1] = Phi(thres[1] - mu) - Phi(-pi() - mu);
    for (k in 2:len_theta - 1){
      out_theta[k] = Phi(thres[k] - mu) - Phi(thres[k - 1] - mu);
    }
    out_theta[len_theta] = Phi(pi() - mu) - Phi(thres[len_theta - 1] - mu);
...
parameters {
  ...
  ordered[NBlocs] raw_thresholds[NElections];
...
transformed parameters {
  ...
  vector[NBlocs - 1] thresholds[NElections];
  ...
  for (j in 1:NElections){
    thresholds[j] = 2.0 * atan(raw_thresholds[j]);
  }
  ...
  mu = 2 * atan(rows_dot_product(XNation_, b_national[incumbency,])[id_Obs_elections] + XDepartment_miss * b_department);

  for (j in 1:NObs){
    {
      theta[j, included_blocs[id_Obs_elections[j]][1:NBlocs_Elections[id_Obs_elections[j]]]] =
        compute_theta_circle(mu[j], thresholds[id_Obs_elections[j], 1:(NBlocs_Elections[id_Obs_elections[j]] - 1)])';
    }
  }
}
model {
