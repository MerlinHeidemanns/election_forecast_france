functions {
  vector compute_theta(real mu, vector thres){
    ...
    for (k in 2:len_theta){
      out_theta[k - 1] = ((2.0 * atan((mu - thres[k - 1])) -
...
parameters {
  positive_ordered[NBlocs - 1] raw_thresholds[NElections];
...
transformed parameters {
  ...
  vector[NBlocs] thresholds[NElections];
  ...
  for (j in 1:NElections){
    thresholds[j, 1] = - 5.0;
    thresholds[j, 2:NBlocs] = raw_thresholds[j] - 5.0;
  }
  ...
  for (j in 1:NObs){
    {
      theta[j, included_blocs[id_Obs_elections[j]][1:NBlocs_Elections[id_Obs_elections[j]]]] =
        compute_theta(mu[j], thresholds[id_Obs_elections[j], 1:NBlocs_Elections[id_Obs_elections[j]]])';
    }
  }
}
model {
  ...
  for (j in 1:NElections) thresholds[j] ~ student_t(3, 0, 3);
