...
transformed data {
  ...
  for (j in 2:m2_N){
    m2_sqrt_t[j - 1] = sqrt(m2_x[j] - m2_x[j - 1]);
  }
  m1_supvec_A[1] = 1;
...
parameters {
  ...
  // M2
  vector<lower = 0>[m1_NBlocs - 1]                  m2_sigma_cholesky;
  matrix[m1_NBlocs - 1, m2_N]            m2_raw_pred;
...
transformed parameters {
  ...
  // M2
  cholesky_factor_cov[m1_NBlocs - 1] cholesky_Omega =
                              diag_pre_multiply(m2_sigma_cholesky, m1_L_Omega);

  matrix[m1_NBlocs, m2_N]      m2_pred;
  ...
  // M2
  m2_pred[2:m1_NBlocs, 1] = m2_mu + cholesky_Omega * m2_raw_pred[, 1];
  for (j in 2:m2_N){
    m2_pred[2:m1_NBlocs, j] = m2_sqrt_t[j - 1] * m2_pred[2:m1_NBlocs, j - 1] + cholesky_Omega * m2_raw_pred[, j];
  }
  m2_pred[1] = rep_row_vector(0.0, m2_N);
...
model {
  ...
  m2_sigma_cholesky ~ normal(0, 0.5);
  m2_mu ~ normal(0, 2);

  // Likelihood
  for (j in 1:m2_N){
    {
      vector[m2_NMiss[j]] theta = softmax(m2_pred[m2_miss[j, 1:m2_NMiss[j]], j] + m2_quality[m2_miss[j, 1:m2_NMiss[j]], j]);
...
generated quantities {
...

  {
    ...
    m2_prediction_new = to_vector(normal_rng(
      softmax(m2_pred[m1_miss_results[m1_T2, 1:m1_NMiss_results[m1_T2]],m2_N1 + 1] +
