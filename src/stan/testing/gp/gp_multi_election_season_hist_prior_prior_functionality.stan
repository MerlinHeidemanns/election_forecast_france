...
data {
  ...
  vector[T2 - 1] abstention_share_results;
  ...
  vector[m2_N1] m2_abstention_share;
}
transformed data {
  real delta = 1e-9;
  int T = T1 * T2;
...
transformed parameters {
...

  {
    profile("gp_exp_quad_cov"){
      ...
      for (i in 1:T2){
        f[locations, 1 + (i - 1) * 16:i * 16] = L_Sigma * eta[i] * chol_K;
      }
      f[5] = zerosT;
      // M2
  ...
  // M2
  for (j in 1:D) m2_quality[j] = m2_raw_quality[j] * m2_sigma_quality[j];
...
model {
  ...
  lambda ~ dirichlet(rep_vector(1, D));
      ...
      for (j in 1:N1){
        {
          ...
          y[miss[j, 1:NMiss[j]], j] ~ multinomial(
            theta[1:NMiss[j]] + ((lambda - divide_D) * abstention_share[j])
          );
        }
      }
    }
  ...
  for (j in 1:(T2 - 1)){
    {
      ...
      vector[NMiss[j]] theta = softmax(f[,result_indicator[j]][index] +
                                       xi[index,j]);
      y_results[miss_results[j, 1:NMiss_results[j]], j] ~ multinomial(theta + (lambda[miss_results[j, 1:NMiss_results[j]]]) * abstention_share_results[j] - mean((lambda[miss_results[j, 1:NMiss_results[j]]]) * abstention_share_results[j]));
    }
  }
  ...
  for (j in 1:m2_N1){
    {
      vector[m2_NMiss[j]] tmp_lambda = lambda[m2_miss[j, 1:m2_NMiss[j]]]/sum(lambda[m2_miss[j, 1:m2_NMiss[j]]]);
      vector[m2_NMiss[j]] m2_theta = softmax(m2_f[, j] +
                                             m2_quality[,j])[m2_miss[j, 1:m2_NMiss[j]]] +
                                    (tmp_lambda - mean(tmp_lambda)) * m2_abstention_share[j]);
      m2_y[j, m2_miss[j, 1:m2_NMiss[j]]] ~ multinomial(m2_theta);
    }
  }
//
//   // Partial pooling
  {
    vector[NMiss_results[T2]] tmp_lambda = lambda[miss_results[T2, 1:NMiss_results[T2]]]/sum(lambda[miss_results[T2, 1:NMiss_results[T2]]]);
    vector[NMiss_results[T2]] theta =
      ...
      (tmp_lambda - mean(tmp_lambda)) * 0.22;
    prediction ~ dirichlet(100000 * theta);
    vector[NMiss_results[T2]] pred = softmax(f[,T2 * T1][miss_results[T2, 1:NMiss_results[T2]]] +
                                             xi[miss_results[T2, 1:NMiss_results[T2]],T2]) +
      (tmp_lambda - mean(tmp_lambda)) * 0.22;
    prediction ~ dirichlet(100000 * pred);
  }
}
